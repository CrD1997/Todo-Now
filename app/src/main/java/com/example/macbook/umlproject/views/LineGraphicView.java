...
public class LineGraphicView extends View
{
    ...
    {
        canvas.drawLine(blwidh, bheight - (bheight / spacingHeight) * 0 + marginTop, (canvasWidth - blwidh),
                bheight - (bheight / spacingHeight) * 0 + marginTop, mPaint);// Y坐标
        drawText(String.valueOf(averageValue * 0), blwidh / 2, bheight - (bheight / spacingHeight) * 0 + marginTop,
                canvas);
        mPaint.setColor(res.getColor(R.color.colorNo));
        for (int i = 1; i < spacingHeight + 1; i++)
        {
    ...
    {
        xList.add(blwidh + (canvasWidth - blwidh) / yRawData.size() * 0);
        canvas.drawLine(blwidh + (canvasWidth - blwidh) / yRawData.size() * 0, marginTop, blwidh
                + (canvasWidth - blwidh) / yRawData.size() * 0, bheight + marginTop, mPaint);
        drawText(xRawDatas.get(0), blwidh + (canvasWidth - blwidh) / yRawData.size() * 0, bheight + dip2px(26),
                canvas);// X坐标
        mPaint.setColor(res.getColor(R.color.colorNo));
